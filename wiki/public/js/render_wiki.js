...
window.RenderWiki = class RenderWiki extends Wiki {
	...
	set_edit_mode() {
		...
		$('.sidebar-edit-mode-btn, .sidebar-view-mode-btn').on('click', () => {
	...
	add_wiki_sidebar(title) {
		const $new_page = this.get_wiki_sidebar_html(title);
		...
		searchInput.on(
			...
			frappe.utils.debounce(() => {
				...
				frappe
					...
					.then((res) => {
						const results = res.message.docs || [];
	...
	setup_page_settings() {
		$('.update-page-settings-button').on('click', () => {
			const name = $('[name="wiki-page-name"]').val();
