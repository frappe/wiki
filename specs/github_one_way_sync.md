...
- **TB4b — OAuth connect + repo picker + engine threading.** The user-facing connect-account round-trip, the installation→repo→branch picker replacing the create-space plain-text inputs, `github_installation_id` on `Wiki Space`, and the engine minting+passing the installation token so private repos sync end-to-end.
...
- [ ] TB4b — OAuth connect + repo picker + engine token threading (private repos sync end-to-end)
- [ ] TB5 — Sync log & status panel
- [ ] TB6 — Real-time webhook sync
