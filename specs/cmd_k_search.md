...
| Matches | Content and title, FTS | Titles |
...
| 3 | Matching | Title `LIKE %query%` server-side once the query has 2 characters, debounced 300ms, 20 rows, most recently modified first, with the space name and route joined in. The client re-ranks with `fuzzysort`, already a dependency, so a prefix match beats a mid-word one. Stale rows from an earlier query are re-ranked against the current one, so rows that no longer match drop out without tracking. |
...
| 7 | Rendering | frappe-ui `Dialog` (`position="top"`, `bare`), a plain combobox input, a `role="listbox"` result list and `KeyboardShortcut` footer hints: Gameplan's structure, with its row metrics (`px-2 py-2`, `mr-3` icons, right-aligned suffix). Unpublished pages and spaces carry an eye-off icon. |
...
| # | Gameplan | Here | Why |
|---|----------|------|-----|
| 1 | Rows show the hit's relative modified time | Rows show the space the page lives in | A page title repeats across spaces ("Getting Started" in four of them); the space is what tells them apart. |
