  ...
  <div class="flex flex-col gap-4 p-4 h-full">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <FormControl type="text" v-model="searchQuery" :placeholder="__('Search spaces...')">
          <template #prefix>
    ...
    <div class="flex-1 overflow-auto">
      <ListView :columns="columns" :rows="spaces.data || []" :options="{
          selectable: false,
          showTooltip: true,
          resizeColumn: false,
        ...
        <template #cell="{ item, column }">
          <Badge v-if="column.key === 'is_published'" variant="subtle" :theme="item ? 'green' : 'orange'" size="sm"
...
<script setup>
  ...
  const spaces = createListResource({
    doctype: "Wiki Space",
    fields: ["name", "space_name", "route", "root_group", "is_published", "owner"],
