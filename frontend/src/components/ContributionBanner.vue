<template>
  <SpaceChromeBar v-if="crStore.isChangeRequestMode" class="contribution-banner" :space-name="spaceName"
    :space-route="spaceRoute" :bar-class="bannerClass" @open-settings="emit('open-settings')">
    <template #badge>
    ...
    <template #meta>
      <span v-if="reviewFeedback" class="truncate text-xs text-ink-red-8 min-w-0" :title="reviewFeedback">
        {{ reviewFeedback }}
      </span>
    </template>
    <template #actions>
      <Badge v-if="syncStateLabel" variant="subtle" :theme="syncStateTheme" size="md" :title="syncStateTitle">
        {{ syncStateLabel }}
      </Badge>

      <Button v-if="showReloadLatest" variant="outline" size="sm" :loading="reloading"
        :title="__('Discard local failed changes and adopt server state')" @click="onReloadLatest">
        {{ __('Reload latest') }}
      </Button>

      <template v-if="changeRequestStatus === 'Draft' || changeRequestStatus === 'Changes Requested'">
        <Button v-if="canShowMerge" size="sm" :loading="crStore.isMerging" :disabled="mergeDisabledComputed"
          :title="mergeButtonTitle" @click="$emit('merge')">
          {{ __('Publish') }}
        </Button>
        <Button v-if="crStore.changeCount > 0" size="sm" :loading="crStore.isSubmitting" :disabled="submitDisabled"
          :title="submitButtonTitle" @click="showSubmitConfirmDialog = true">
  ...
  <Dialog v-model:open="showChangesDialog" size="lg">
    ...
    <template #default>
      <div class="space-y-3 max-h-[60vh] overflow-y-auto">
        <div v-for="change in crStore.changes" :key="change.doc_key"
          ...
          <div class="flex-1 min-w-0">
            ...
            <p v-if="change.is_external_link && change.external_url" class="text-sm text-ink-gray-5 mt-0.5 truncate">
              <a :href="change.external_url" target="_blank" rel="noopener noreferrer"
  ...
  <Dialog v-model:open="showSubmitConfirmDialog" size="sm">
    ...
    <template #actions="{ close }">
      <div class="flex justify-end gap-2">
        ...
        <Button variant="solid" :loading="crStore.isSubmitting" @click="confirmSubmit(close)">
          {{ __('Submit') }}
        </Button>
      </div>
    </template>
